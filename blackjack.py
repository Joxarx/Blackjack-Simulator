...
class Card:
    ...
    def draw(self, x, y):
        ...
        if self.visible:
            ...
            big_suit = pygame.font.SysFont('arial', 50).render(self.suit, True, color)
            ...
            for i in range(5):
                pygame.draw.circle(screen, GOLD, (x + 50, y + 30 + i*20), 10, 2)
...
class BlackjackGame:
    ...
    def stand(self):
        ...
        while self.dealer.score < 17 and not self.dealer.bust:
            self.dealer.add_card(self.deck.deal())
            time.sleep(0.5)  # Pausa para dramatismo
...
def draw_game(game, buttons):
    ...
    # Cartas del dealer
    dealer_text = font.render(f"Crupier: {game.dealer.score if game.dealer.hand[1].visible else '?'}", True, WHITE)
    ...
    if game.result:
        ...
        screen.blit(result_text, (WIDTH // 2 - result_text.get_width() // 2, 540))
